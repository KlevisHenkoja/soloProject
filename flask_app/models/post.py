...
class Post:
    ...
    @classmethod
    def getAllPosts(cls):
        query= 'SELECT posts.id, description, COUNT(likes.id) as likesNr, users.id as creator_id, email FROM posts LEFT JOIN users on posts.user_id = users.id LEFT JOIN likes on likes.post_id = posts.id GROUP BY posts.id;'
